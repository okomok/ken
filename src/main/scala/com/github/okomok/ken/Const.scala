...
object Const {
    implicit def asFunctor[m]: Functor[({type f[+x] = Const[m, x]})#f] = new Functor[({type f[+x] = Const[m, x]})#f] {
    ...
    implicit def asApplicative[m](implicit i: Monoid[m]): Applicative[({type f[+x] = Const[m, x]})#f] = new Applicative[({type f[+x] = Const[m, x]})#f] {
        private[this] type f[+x] = Const[m, x]
