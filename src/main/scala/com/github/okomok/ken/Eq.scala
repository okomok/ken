...
object _Eq extends EqInstance with EqShortcut {
    ...
    def deriving[nt <: Kind.Newtype0](implicit i: _Eq[nt#oldtype0], j: Newtype0[nt#apply0, nt#oldtype0, _]): _Eq[nt#apply0] = new _Eq[nt#apply0] {
        override val op_=== : op_=== = x => y => i.op_===(j.oldOf(x))(j.oldOf(y))
        override val op_/== : op_/== = x => y => i.op_/==(j.oldOf(x))(j.oldOf(y))
    }

    def weak[nt <: Kind.Newtype0](implicit i: _Eq[nt#apply0], j: Newtype0[nt#apply0, nt#oldtype0, _]): _Eq[nt#oldtype0] = deriving[Kind.coNewtype0[nt]](i, j.coNewtype)
