...
private[ken] final class _StateTs[n[+_]](override val inner: Monad[n]) extends MonadTs[n] {
    ...
    object _StateT extends _StateTAs with Kind.FunctionLike {
        ...
        def map[s, m[+_], a, b](f: n[(a, s)] => m[(b, s)])(n: _StateT[s, a]): NewtypeOf[s => m[(b, s)]] = NewtypeOf { f `.` run(n) }

        def `with`[s, a](f: s => s)(n: _StateT[s, a]): _StateT[s, a] = _StateT { run(n) `.` f }
    }

    private[ken] sealed trait _StateTAs0 { this: _StateT.type =>
        implicit def _asNewtype1[s]: Newtype1[({type nt[+a] = _StateT[s, a]})#nt, ({type ot[+a] = s => n[(a, s)]})#ot] = new Newtype1[({type nt[+a] = _StateT[s, a]})#nt, ({type ot[+a] = s => n[(a, s)]})#ot] {
            private type nt[+a] = _StateT[s, a]
            private type ot[+a] = s => n[(a, s)]
            override def newOf[a](ot: Lazy[ot[a]]): nt[a] = _StateT(ot)
            override def oldOf[a](nt: Lazy[nt[a]]): ot[a] = nt.run
        }

        implicit def _asMonadTrans[s]: MonadTrans[({type m[+a] = _StateT[s, a]})#m, n] = new MonadTrans[({type m[+a] = _StateT[s, a]})#m, n] {
