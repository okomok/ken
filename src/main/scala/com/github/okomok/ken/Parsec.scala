...
// Prim
    ...
    def getState[tok, st]: GenParser[tok, st, st] = {
        val i = GenParser.asMonad[tok, st]
        import i.`for`
    ...
    sealed abstract class GenParser[tok, st, +a] {
    ...
    object Parser {
        ...
        implicit val asMonad: MonadPlus[({type m[+x] = GenParser[Char, Unit, x]})#m] = GenParser.asMonad[Char, Unit]
    }
    ...
    object GenParser {
        implicit def asMonad[tok, st]: MonadPlus[({type m[+x] = GenParser[tok, st, x]})#m] = new MonadPlus[({type m[+x] = GenParser[tok, st, x]})#m] {
    ...
    /** always fail **/
    def parsecZero[tok, st]: GenParser[tok, st, Nothing] = {
        Parser { (state: State[tok, st]) =>
...
// Combinators
    ...
    def skipMany[tok, st](p: GenParser[tok, st, _]): GenParser[tok, st, Unit] = {
        ...
        lazy val scan: GenParser[tok, st, Unit] = ( for { _ <- p: m[_]; _ <- scan: m[Unit] } yield () ) <|> `return`()(asMonad[m])
        scan
    }
    */
