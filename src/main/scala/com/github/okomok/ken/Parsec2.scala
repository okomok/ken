...
// Prim
    ...
    def getState[tok, st]: GenParser[tok, st, st] = {
        val i = GenParser._asMonadPlus[tok, st]
        import i.`for`
    ...
    object Parser extends Kind.AbstractFunction1 {
        ...
        implicit val _asMonadPlus: MonadPlus[({type m[+x] = GenParser[Char, Unit, x]})#m] = GenParser._asMonadPlus[Char, Unit]
    }
...
// Error
    ...
    def messageCompare(msg1: Message_)(msg2: Message_): Ordering = {
        Int.compare(messageToEnum(msg1))(messageToEnum(msg2))
    }
...
// Combinators
    ...
    def skipMany[tok, st](p: GenParser[tok, st, _]): GenParser[tok, st, Unit] = {
        ...
        lazy val scan: GenParser[tok, st, Unit] = ( for { _ <- p: m[_]; _ <- scan: m[Unit] } yield () ) <|> `return`()(_asMonadPlus[m])
        scan
    }
    */
