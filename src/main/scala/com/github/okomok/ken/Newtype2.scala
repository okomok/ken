...
trait Newtype2[nt[-_, +_], ot[-_, +_]] extends Typeclass with Kind.AbstractNewtype2 { outer =>
    ...
    // Core
    //
    def new2[a, b](ot: => ot[a, b]): nt[a, b]
    def old2[a, b](nt: => nt[a, b]): ot[a, b]

    // Extra
    //
    final def run[a, b](nt: nt[a, b]): ot[a, b] = old2(nt)

    def dual: Newtype2[ot, nt] = new Newtype2[ot, nt] {
        override def old2[a, b](ot: => ot[a, b]): nt[a, b] = outer.new2(ot)
        override def new2[a, b](nt: => nt[a, b]): ot[a, b] = outer.old2(nt)
    }
}
...
trait Newtype2Proxy[nt[-_, +_], ot[-_, +_]] extends Newtype2[nt, ot] with Proxy {
    ...
    override def new2[a, b](ot: => ot[a, b]): nt[a, b] = self.new2(ot)
    override def old2[a, b](nt: => nt[a, b]): ot[a, b] = self.old2(nt)
