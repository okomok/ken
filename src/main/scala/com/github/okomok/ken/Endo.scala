...
final case class Endo[a](override val get: a => a) extends NewtypeOf[a => a] with Kind.AbstractNewtype0 with Kind.alwaysThis {
...
object Endo {
    implicit def _asNewtype0[a]: Newtype0[Endo[a], a => a] = new Newtype0[Endo[a], a => a] {
        ...
        override def newOf(ot: Lazy[ot]): nt = Endo(ot)
        override def oldOf(nt: Lazy[nt]): ot = nt.run
    ...
    implicit def _asMonoid[a]: Monoid[Endo[a]] = new Monoid[Endo[a]] {
        ...
        override val mappend: m => Lazy[m] => m = x => y => Endo(x.run compose y.!.run)
    }
}
