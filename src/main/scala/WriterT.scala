...
final class _WriterTs[n[+_]](val inner: Monad[n]) {
    ...
    object _WriterT extends LowPriorityImplicits {
        def apply[w, a](rep: n[(a, w)]): _WriterT[w, a] = rep

        // implicit def from[w, a](n: WriterMonadT[w, n, a]): _WriterT[w, a] = _WriterT { n.run }

        def run[w, a](n: _WriterT[w, a]): n[(a, w)] = n

        def exec[w, a](n: _WriterT[w, a]): n[w] = {
            import inner.`for`
