...
trait Applicative[f[_]] extends Functor[f] {
    ...
    private[ken] class _Op_<*>[a, b](x: f[a => b]) {
...
object Applicative {
    implicit val Option: Applicative[Option] = detail._Option
    implicit val List: Applicative[List] = detail._List
    implicit def Function1[c]: Applicative[({type f[a] = c => a})#f] = new detail._Function1[c]
}
